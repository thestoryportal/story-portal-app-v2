...
function createDefaultPlasmaLayer(
  index: number,
  overrides: Partial<PlasmaLayerConfig> = {}
): PlasmaLayerConfig {
  return {
...
function createDefaultBoltLayer(
  index: number,
  overrides: Partial<BoltLayerConfig> = {}
): BoltLayerConfig {
  return {
...
function createDefaultRimLayer(
  index: number,
  overrides: Partial<RimLayerConfig> = {}
): RimLayerConfig {
  return {
...
function createDefaultOuterGlowLayer(
  index: number,
  overrides: Partial<OuterGlowLayerConfig> = {}
): OuterGlowLayerConfig {
  return {
...
function useLayerState() {
  ...
  const updateLayer = useCallback(
    <T extends keyof LayerState>(type: T, id: string, updates: Partial<LayerState[T][number]>) => {
      setLayers((prev) => ({
        ...prev,
        [type]: prev[type].map((layer) => (layer.id === id ? { ...layer, ...updates } : layer)),
      }))
    },
    []
  )
  ...
  return folder(
    {
      ...
      '⚙️ Settings': folder({
        ...
        enabled: {
          value: layer.enabled,
          label: 'Enabled',
          onChange: (v: boolean) => onUpdate({ enabled: v }),
        },
        zDepth: {
          value: layer.zDepth,
          min: -5,
          ...
          step: 0.1,
          label: 'Z-Depth',
          onChange: (v: number) => onUpdate({ zDepth: v }),
        },
        baseOpacity: {
          value: layer.baseOpacity,
          min: 0,
          max: 1,
          step: 0.01,
          label: 'Base Opacity',
          onChange: (v: number) => onUpdate({ baseOpacity: v }),
        },
        noiseOffset: {
          value: layer.noiseOffset,
          min: 0,
          max: 5,
          step: 0.1,
          label: 'Noise Offset',
          onChange: (v: number) => onUpdate({ noiseOffset: v }),
        },
      }),
      ...
      '🌈 Colors': folder(
        {
          coreColor: {
            value: layer.coreColor,
            label: '☀️ Core',
            onChange: (v: string) => onUpdate({ coreColor: v }),
          },
          innerColor: {
            value: layer.innerColor,
            label: '🔥 Inner',
            onChange: (v: string) => onUpdate({ innerColor: v }),
          },
          midColor: {
            value: layer.midColor,
            label: '🔶 Mid',
            onChange: (v: string) => onUpdate({ midColor: v }),
          },
          outerColor: {
            value: layer.outerColor,
            label: '🌑 Outer',
            onChange: (v: string) => onUpdate({ outerColor: v }),
          },
          hotAmberColor: {
            value: layer.hotAmberColor,
            label: '✨ Accent',
            onChange: (v: string) => onUpdate({ hotAmberColor: v }),
          },
        },
        { collapsed: true }
      ),
      ...
      '🌀 Chaos': folder(
        {
          colorChaos: {
            value: layer.colorChaos,
            min: 0,
            max: 1,
            step: 0.05,
            label: 'Color',
            onChange: (v: number) => onUpdate({ colorChaos: v }),
          },
          opacityChaos: {
            value: layer.opacityChaos,
            min: 0,
            max: 1,
            step: 0.05,
            label: 'Opacity',
            onChange: (v: number) => onUpdate({ opacityChaos: v }),
          },
          centerChaos: {
            value: layer.centerChaos,
            min: 0,
            max: 1,
            step: 0.05,
            label: 'Center',
            onChange: (v: number) => onUpdate({ centerChaos: v }),
          },
          chaosSpeed: {
            value: layer.chaosSpeed,
            min: 0,
            max: 2,
            step: 0.1,
            label: 'Speed',
            onChange: (v: number) => onUpdate({ chaosSpeed: v }),
          },
          chaosScale: {
            value: layer.chaosScale,
            min: 0.5,
            max: 10,
            step: 0.5,
            label: 'Scale',
            onChange: (v: number) => onUpdate({ chaosScale: v }),
          },
        },
        { collapsed: true }
      ),
      ...
      '🎚️ HSL': folder(
        {
          hueShift: {
            ...
            step: 5,
            label: 'Hue Shift',
            onChange: (v: number) => onUpdate({ hueShift: v }),
          },
          saturationMult: {
            value: layer.saturationMult,
            min: 0,
            max: 2,
            step: 0.1,
            label: 'Saturation',
            onChange: (v: number) => onUpdate({ saturationMult: v }),
          },
          brightnessMult: {
            value: layer.brightnessMult,
            min: 0,
            max: 2,
            step: 0.1,
            label: 'Brightness',
            onChange: (v: number) => onUpdate({ brightnessMult: v }),
          },
          colorTemperature: {
            value: layer.colorTemperature,
            min: -1,
            max: 1,
            step: 0.1,
            label: 'Temperature',
            onChange: (v: number) => onUpdate({ colorTemperature: v }),
          },
        },
        { collapsed: true }
      ),
      ...
      '🌀 Vortex': folder(
        {
          vortexEnabled: {
            value: layer.vortexEnabled,
            label: 'Enable',
            onChange: (v: boolean) => onUpdate({ vortexEnabled: v }),
          },
          vortexSpeed: {
            value: layer.vortexSpeed,
            min: 0,
            max: 0.5,
            step: 0.01,
            label: 'Speed',
            onChange: (v: number) => onUpdate({ vortexSpeed: v }),
          },
          vortexTwist: {
            value: layer.vortexTwist,
            min: 0,
            max: 8,
            step: 0.1,
            label: 'Twist',
            onChange: (v: number) => onUpdate({ vortexTwist: v }),
          },
          pullStrength: {
            value: layer.pullStrength,
            min: 0,
            max: 0.5,
            step: 0.01,
            label: 'Pull',
            onChange: (v: number) => onUpdate({ pullStrength: v }),
          },
          pullPulse: {
            value: layer.pullPulse,
            min: 0,
            max: 2,
            step: 0.05,
            label: 'Pulse',
            onChange: (v: number) => onUpdate({ pullPulse: v }),
          },
        },
        { collapsed: true }
      ),
      ...
      '👆 Tendrils': folder(
        {
          tendrilsEnabled: {
            value: layer.tendrilsEnabled,
            label: 'Enable',
            onChange: (v: boolean) => onUpdate({ tendrilsEnabled: v }),
          },
          numTendrils: {
            value: layer.numTendrils,
            min: 2,
            max: 24,
            step: 1,
            label: 'Count',
            onChange: (v: number) => onUpdate({ numTendrils: v }),
          },
          tendrilSharpness: {
            value: layer.tendrilSharpness,
            min: 1,
            max: 10,
            step: 0.5,
            label: 'Sharpness',
            onChange: (v: number) => onUpdate({ tendrilSharpness: v }),
          },
          tendrilSpeed: {
            value: layer.tendrilSpeed,
            min: 0,
            max: 1,
            step: 0.05,
            label: 'Speed',
            onChange: (v: number) => onUpdate({ tendrilSpeed: v }),
          },
          tendrilOpacity: {
            value: layer.tendrilOpacity,
            min: 0,
            max: 1,
            step: 0.05,
            label: 'Opacity',
            onChange: (v: number) => onUpdate({ tendrilOpacity: v }),
          },
        },
        { collapsed: true }
      ),
      ...
      '☁️ Clouds': folder(
        {
          cloudScale: {
            value: layer.cloudScale,
            min: 0.5,
            max: 8,
            step: 0.1,
            label: 'Cloud Size',
            onChange: (v: number) => onUpdate({ cloudScale: v }),
          },
          cloudSpeed: {
            value: layer.cloudSpeed,
            min: 0,
            max: 0.3,
            step: 0.01,
            label: 'Cloud Speed',
            onChange: (v: number) => onUpdate({ cloudSpeed: v }),
          },
          wispScale: {
            value: layer.wispScale,
            min: 1,
            max: 12,
            step: 0.5,
            label: 'Wisp Size',
            onChange: (v: number) => onUpdate({ wispScale: v }),
          },
          wispSpeed: {
            value: layer.wispSpeed,
            min: 0,
            max: 0.5,
            step: 0.01,
            label: 'Wisp Speed',
            onChange: (v: number) => onUpdate({ wispSpeed: v }),
          },
          emberScale: {
            value: layer.emberScale,
            min: 4,
            max: 30,
            step: 1,
            label: 'Ember Size',
            onChange: (v: number) => onUpdate({ emberScale: v }),
          },
          emberSpeed: {
            value: layer.emberSpeed,
            min: 0,
            max: 0.5,
            step: 0.01,
            label: 'Ember Speed',
            onChange: (v: number) => onUpdate({ emberSpeed: v }),
          },
        },
        { collapsed: true }
      ),
      ...
      '💡 Core': folder(
        {
          coreGlowStrength: {
            value: layer.coreGlowStrength,
            min: 0,
            max: 1.5,
            step: 0.05,
            label: 'Strength',
            onChange: (v: number) => onUpdate({ coreGlowStrength: v }),
          },
          coreGlowFalloff: {
            value: layer.coreGlowFalloff,
            min: 0.5,
            max: 5,
            step: 0.1,
            label: 'Falloff',
            onChange: (v: number) => onUpdate({ coreGlowFalloff: v }),
          },
        },
        { collapsed: true }
      ),
      ...
      '📐 Edge': folder(
        {
          edgeFalloffStart: {
            value: layer.edgeFalloffStart,
            min: 0,
            max: 1,
            step: 0.05,
            label: 'Start',
            onChange: (v: number) => onUpdate({ edgeFalloffStart: v }),
          },
          edgeFalloffEnd: {
            value: layer.edgeFalloffEnd,
            min: 0.5,
            max: 1.5,
            step: 0.05,
            label: 'End',
            onChange: (v: number) => onUpdate({ edgeFalloffEnd: v }),
          },
          falloffCurve: {
            value: layer.falloffCurve,
            min: 0.1,
            max: 2,
            step: 0.1,
            label: 'Curve',
            onChange: (v: number) => onUpdate({ falloffCurve: v }),
          },
        },
        { collapsed: true }
      ),
      ...
      '⚙️ Master': folder(
        {
          glowMult: {
            value: layer.glowMult,
            min: 0.1,
            max: 5,
            step: 0.1,
            label: 'Glow',
            onChange: (v: number) => onUpdate({ glowMult: v }),
          },
          opacityMult: {
            value: layer.opacityMult,
            min: 0,
            max: 3,
            step: 0.05,
            label: 'Opacity',
            onChange: (v: number) => onUpdate({ opacityMult: v }),
          },
          globalSpeed: {
            value: layer.globalSpeed,
            min: 0,
            max: 3,
            step: 0.1,
            label: 'Speed',
  ...
  return folder(
    {
      ...
      count: {
        ...
        step: 1,
        label: 'Count',
        onChange: (v: number) => onUpdate({ count: v }),
      },
      primaryColor: {
        value: layer.primaryColor,
        label: 'Primary Color',
        onChange: (v: string) => onUpdate({ primaryColor: v }),
      },
      secondaryColor: {
        value: layer.secondaryColor,
        label: 'Secondary Color',
        onChange: (v: string) => onUpdate({ secondaryColor: v }),
      },
      glowColor: {
        value: layer.glowColor,
        label: 'Glow Color',
        onChange: (v: string) => onUpdate({ glowColor: v }),
      },
      primaryOpacity: {
        value: layer.primaryOpacity,
        min: 0,
        max: 1,
        step: 0.05,
        label: 'Primary Opacity',
        onChange: (v: number) => onUpdate({ primaryOpacity: v }),
      },
      secondaryOpacity: {
        value: layer.secondaryOpacity,
        min: 0,
        max: 1,
        step: 0.05,
        label: 'Secondary Opacity',
        onChange: (v: number) => onUpdate({ secondaryOpacity: v }),
      },
      glowIntensity: {
        ...
        onChange: (v: number) => onUpdate({ glowIntensity: v }),
      },
      thicknessScale: {
        value: layer.thicknessScale,
        min: 0.1,
        max: 3,
        step: 0.1,
        label: 'Thickness',
        onChange: (v: number) => onUpdate({ thicknessScale: v }),
      },
      lengthScale: {
        value: layer.lengthScale,
        min: 0.1,
        max: 2,
        step: 0.1,
        label: 'Length',
        onChange: (v: number) => onUpdate({ lengthScale: v }),
      },
      roughness: {
        ...
        step: 0.05,
        label: 'Roughness',
        onChange: (v: number) => onUpdate({ roughness: v }),
      },
      straightness: {
        value: layer.straightness,
        min: 0,
        max: 1,
        step: 0.05,
        label: 'Straightness',
        onChange: (v: number) => onUpdate({ straightness: v }),
      },
  ...
  return folder(
    {
      ...
      innerEdge: {
        value: layer.innerEdge,
        min: 0,
        max: 0.9,
        step: 0.05,
        label: 'Inner Edge',
        onChange: (v: number) => onUpdate({ innerEdge: v }),
      },
      outerEdge: {
        value: layer.outerEdge,
        min: 0.5,
        max: 1.1,
        step: 0.05,
        label: 'Outer Edge',
        onChange: (v: number) => onUpdate({ outerEdge: v }),
      },
      chromaticAberration: {
        ...
        step: 0.005,
        label: 'Chromatic',
        onChange: (v: number) => onUpdate({ chromaticAberration: v }),
      },
      flicker: {
        value: layer.flicker,
        min: 0,
        ...
        step: 0.05,
        label: 'Flicker',
        onChange: (v: number) => onUpdate({ flicker: v }),
      },
      color1: {
        value: layer.color1,
        label: 'Color 1',
        onChange: (v: string) => onUpdate({ color1: v }),
      },
      color2: {
        value: layer.color2,
        label: 'Color 2',
        onChange: (v: string) => onUpdate({ color2: v }),
      },
      zDepth: {
        value: layer.zDepth,
        min: -5,
  ...
  return folder(
    {
      ...
      size: {
        ...
        step: 0.01,
        label: 'Size',
        onChange: (v: number) => onUpdate({ size: v }),
      },
      intensity: {
      ...
      softness: {
        ...
        step: 0.05,
        label: 'Softness',
        onChange: (v: number) => onUpdate({ softness: v }),
      },
      zDepth: {
        value: layer.zDepth,
        min: -5,
...
function generateAllLayerControls(
  ...
  return {
    ...
    '🔥 Plasma Layers': folder({
      [`➕ Add Plasma (${layers.plasma.length}/${MAX_LAYERS_PER_TYPE})`]: button(() =>
        addLayer('plasma')
      ),
      ...plasmaLayerControls,
    }),
    ...
    // Bolt Layers folder
    '⚡ Bolt Layers': folder({
      [`➕ Add Bolts (${layers.bolts.length}/${MAX_LAYERS_PER_TYPE})`]: button(() =>
        addLayer('bolts')
      ),
      ...boltLayerControls,
    }),
    ...
    '🌟 Outer Glow Layers': folder({
      [`➕ Add Glow (${layers.outerGlow.length}/${MAX_LAYERS_PER_TYPE})`]: button(() =>
        addLayer('outerGlow')
      ),
    ...
    bloomThreshold: {
      value: cfg.r3fBloomLuminanceThreshold,
      min: 0,
      max: 1,
      step: 0.05,
...
function LightningBolt({
  ...
  useFrame((state) => {
    ...
    if (meshRef.current && controlsRef.current) {
      ...
      // Update color
      mat.color.setRGB(colorRgb[0] * glowMult, colorRgb[1] * glowMult, colorRgb[2] * glowMult)
...
function PlasmaBackground({
  ...
  useFrame((state) => {
    ...
    if (materialRef.current && controlsRef.current) {
      ...
      mat.uniforms.u_outerColor.value.set(
        ctrl.outerColor[0],
        ctrl.outerColor[1],
        ctrl.outerColor[2]
      )
    ...
    <>
                  ...
                  if (mat && layerRef.current) {
                    const updateRim = () => {
                      ...
                      if (ctrl) {
                        ...
                        mat.uniforms.u_color1.value.set(
                          ctrl.color1[0],
                          ctrl.color1[1],
                          ctrl.color1[2]
                        )
                        mat.uniforms.u_color2.value.set(
                          ctrl.color2[0],
                          ctrl.color2[1],
                          ctrl.color2[2]
                        )
      ...
      {layers.outerGlow
        ...
        .map((layer) => {
          ...
          if (!layerRef) return null
          return <OuterGlowLayer key={layer.id} startTime={startTime} controlsRef={layerRef} />
        })}
      ...
      <EffectComposer>
        <Bloom
          intensity={
            (layerControls as unknown as Record<string, number>).bloomIntensity ??
            cfg.r3fBloomIntensity
          }
          luminanceThreshold={
            (layerControls as unknown as Record<string, number>).bloomThreshold ??
            cfg.r3fBloomLuminanceThreshold
          }
          luminanceSmoothing={cfg.r3fBloomLuminanceSmoothing}
          radius={
            (layerControls as unknown as Record<string, number>).bloomRadius ?? cfg.r3fBloomRadius
          }
        />
      </EffectComposer>
    </>
