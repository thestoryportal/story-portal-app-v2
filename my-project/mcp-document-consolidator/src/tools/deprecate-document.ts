...
export function createDeprecateDocumentTool(deps: DeprecateToolDependencies) {
  return {
    ...
    async execute(input: DeprecateDocumentInput): Promise<DeprecateDocumentOutput> {
      ...
      await updateElasticsearchDeprecation(
        deps.es,
        deps.esIndex,
        input.document_id,
        true
...
async function updateElasticsearchDeprecation(
  es: ElasticsearchClient,
  index: string,
  documentId: string,
  deprecated: boolean
  ...
  try {
    // Update all sections belonging to this document
    await es.updateByQuery({
      index,
      body: {
        script: {
          source: 'ctx._source.deprecated = params.deprecated',
          params: { deprecated }
        },
        query: {
    ...
    // Log but don't fail - ES update is not critical
    console.error(`Failed to update ES deprecation status for document ${documentId}`);
  }
}
