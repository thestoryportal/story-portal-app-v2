...
// Get the directory of this module (works correctly when run from any cwd)
...
export class EmbeddingPipeline {
  ...
  constructor(config?: string | EmbeddingPipelineConfig) {
    if (typeof config === 'string') {
      ...
      // Use module directory, not cwd (supports running from any directory)
      this.pythonScriptPath = path.join(__dirname, '..', '..', 'python', 'embedding_service.py');
    } else {
      this.pythonPath = 'python3';
      this.modelName = 'all-MiniLM-L6-v2';
      // Use module directory, not cwd (supports running from any directory)
  ...
  async initialize(): Promise<void> {
    ...
    // Wait for the process to be ready (120s timeout for first-time model download)
