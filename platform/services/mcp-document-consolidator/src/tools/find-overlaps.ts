...
export function createFindOverlapsTool(deps: FindOverlapsToolDependencies) {
  ...
  return {
    ...
    async execute(input: unknown): Promise<FindOverlapsOutput> {
      // Validate input against schema
      const parseResult = FindOverlapsInputSchema.safeParse(input);
      ...
      const documentIds = await resolveScope(deps.db, validatedInput.scope, validatedInput.include_archived);

      // 2. Load all sections for scoped documents
      const sections = await deps.db.sections.findByDocumentIds(documentIds);

      // 3. Get sections with embeddings from PostgreSQL (pgvector)
      const sectionsWithEmbeddings = await deps.db.sections.findWithEmbeddingsByDocumentIds(documentIds);

      // 4. Cluster similar sections using pgvector embeddings
      ...
      const filteredConflicts = validatedInput.conflict_types
        ? conflicts.filter(c => validatedInput.conflict_types!.includes(c.conflict_type as ConflictType))
        : conflicts;
...
function clusterBySimilarity(
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    if (assigned.has(sectionA.id)) continue;

    const clusterMembers: Array<{ document_id: string; section_id: string; header: string }> = [
      { document_id: sectionA.document_id, section_id: sectionA.id, header: sectionA.header }
    ];

    for (let j = i + 1; j < sections.length; j++) {
      const sectionB = sections[j];
      if (assigned.has(sectionB.id)) continue;

      // Calculate cosine similarity
      const similarity = cosineSimilarity(sectionA.embedding, sectionB.embedding);

      if (similarity >= threshold) {
        clusterMembers.push({
          document_id: sectionB.document_id,
          section_id: sectionB.id,
          header: sectionB.header
        });
        assigned.add(sectionB.id);
      }
    }

    if (clusterMembers.length > 1) {
      assigned.add(sectionA.id);

      // Group by document
      const byDocument = new Map<string, string[]>();
      for (const member of clusterMembers) {
        const existingSections = byDocument.get(member.document_id) || [];
        existingSections.push(member.section_id);
        byDocument.set(member.document_id, existingSections);
      }
      ...
      clusters.push({
        id: uuidv4(),
        theme: sectionA.header || 'Unnamed Section',
