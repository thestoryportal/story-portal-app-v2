...
const ExtractionResponseSchema = z.object({
  claims: z.array(z.object({
    ...
    // Make these fields optional with defaults for models that don't provide them
    confidence: z.number().min(0).max(1).optional().default(0.8),
    start_char: z.number().int().optional().default(0),
    end_char: z.number().int().optional().default(0)
  }))
});
...
export class ClaimExtractor {
  ...
  constructor(llmService: LLMService, model?: string) {
    ...
    this.model = model; // If undefined, llmService will use its default model
  }
...
Output JSON format:
{
  "claims": [
    ...
    try {
      ...
      const modelName = this.model || 'default';
      if (error instanceof z.ZodError) {
        throw new LLMError(modelName, `Failed to parse LLM response: ${error.message}`);
      }
      throw new LLMError(modelName, `Claim extraction failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
