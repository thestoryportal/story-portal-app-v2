...
describe('Get Source of Truth Tool', () => {
  ...
  describe('createGetSourceOfTruthTool', () => {
    ...
    it('should search database with vector query via pgvector', async () => {
      const tool = createGetSourceOfTruthTool({
        db: mockDb as DatabaseService,
        embeddingPipeline: mockEmbeddingPipeline as unknown as import('../../src/ai/embedding-pipeline.js').EmbeddingPipeline,
        llmPipeline: mockLlmPipeline as unknown as import('../../src/ai/llm-pipeline.js').LLMPipeline
      ...
      expect(mockDb.sections?.semanticSearch).toHaveBeenCalled();
    });

    it('should filter deprecated documents when include_deprecated is false', async () => {
      // Set up semantic search to return results from both active and archived docs
      (mockDb.sections?.semanticSearch as Mock).mockResolvedValue([
        { document_id: 'doc-1', section_id: 'sec-1', content: 'Active content', header: 'Header', similarity: 0.95 },
        { document_id: 'doc-2', section_id: 'sec-2', content: 'Archived content', header: 'Header', similarity: 0.90 }
      ]);

      // Set up doc-2 as archived type
      ...
      // Should only include non-archived sources
      const archivedSources = result.sources.filter(s => s.document_id === 'doc-2');
      expect(archivedSources.length).toBe(0);
    });
    ...
    it('should filter by scope when provided', async () => {
      ...
      // Set up semantic search to return only in-scope results
      (mockDb.sections?.semanticSearch as Mock).mockResolvedValue([
        { document_id: inScopeDocId, section_id: 'sec-1', content: 'In scope content', header: 'Header', similarity: 0.95 }
      ]);
    ...
    it('should handle no results from semantic search', async () => {
      // Set up semantic search to return empty results
      (mockDb.sections?.semanticSearch as Mock).mockResolvedValue([]);
