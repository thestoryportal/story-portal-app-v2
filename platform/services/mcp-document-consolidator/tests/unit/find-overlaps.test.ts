...
describe('Find Overlaps Tool', () => {
  ...
  beforeEach(() => {
    ...
    mockDb = {
      ...
      sections: {
        ...
        findWithEmbeddingsByDocumentIds: vi.fn().mockResolvedValue([
          { id: 'sec-1', document_id: 'doc-1', header: 'Section 1', content: 'Test content 1', embedding: [0.1, 0.2, 0.3] },
          { id: 'sec-2', document_id: 'doc-2', header: 'Section 2', content: 'Test content 2', embedding: [0.15, 0.25, 0.35] }
        ]),
        findById: vi.fn()
  ...
  describe('createFindOverlapsTool', () => {
    ...
    it('should detect overlap clusters from similar embeddings', async () => {
      // Set up DB to return similar sections (high similarity via pgvector)
      (mockDb.sections?.findWithEmbeddingsByDocumentIds as Mock).mockResolvedValue([
        { id: 'sec-1', document_id: 'doc-1', header: 'Similar Section', content: 'Test content', embedding: [0.5, 0.5, 0.5] },
        { id: 'sec-2', document_id: 'doc-2', header: 'Similar Section', content: 'Test content similar', embedding: [0.51, 0.49, 0.5] }
      ]);
    ...
    it('should generate recommendations for overlapping content', async () => {
      // Set up DB mock with highly similar content via pgvector
      (mockDb.sections?.findWithEmbeddingsByDocumentIds as Mock).mockResolvedValue([
        { id: 'sec-1', document_id: 'doc-1', header: 'Header', content: 'Content A', embedding: [1, 0, 0] },
        { id: 'sec-2', document_id: 'doc-2', header: 'Header', content: 'Content A copy', embedding: [0.99, 0.01, 0] }
      ]);
